...
{
    if (cfg == NULL) {
        ESP_LOGE(TAG, "gateway_config_load: cfg is NULL");
        return;
    }
    ...
    strncpy(cfg->aggregator_ip, CONFIG_MESH_AGGREGATOR_IP,
            NVS_CFG_IP_MAX - 1);
    cfg->aggregator_ip[NVS_CFG_IP_MAX - 1] = '\0';

    cfg->aggregator_port = (uint16_t)CONFIG_MESH_AGGREGATOR_PORT;

    /* Try to override from NVS namespace "mesh_gw" */
    nvs_handle_t handle;
    esp_err_t err = nvs_open("mesh_gw", NVS_READONLY, &handle);
    if (err != ESP_OK) {
    ...
    /* Aggregator IP */
    len = sizeof(buf);
    if (nvs_get_str(handle, "agg_ip", buf, &len) == ESP_OK && len > 1) {
        strncpy(cfg->aggregator_ip, buf, NVS_CFG_IP_MAX - 1);
        cfg->aggregator_ip[NVS_CFG_IP_MAX - 1] = '\0';
        ESP_LOGI(TAG, "NVS: aggregator_ip=%s", cfg->aggregator_ip);
    }

    /* Aggregator port */
    uint16_t port;
    if (nvs_get_u16(handle, "agg_port", &port) == ESP_OK) {
        cfg->aggregator_port = port;
        ESP_LOGI(TAG, "NVS: aggregator_port=%u", port);
    }
